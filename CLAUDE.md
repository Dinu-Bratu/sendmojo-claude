...
This file is read automatically by Claude Code at the start of every session in this repo. Keep it short — the substance lives in the linked documents below, not duplicated here.
...
2. `docs/CODING_STANDARDS.md`. Follow every selection exactly. If a choice is still unresolved, stop and ask rather than guessing.

## Working agreement

- This repo is one half of a deliberate side-by-side comparison against a Codex-driven implementation of the same spec. Do not try to mirror or peek at decisions made in the other repo — implementation choices here should stand on their own merits.
